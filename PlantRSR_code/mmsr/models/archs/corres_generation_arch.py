...
class CorrespondenceGenerationArch(nn.Module):
    ...
    def forward(self, dense_features, img_ref_hr):
        ...
        for ind in range(img_ref_hr.size(0)):
            ...
            _max_idx, mask1 = feature_match_index(
                feat_in,
                feat_ref,
            ...
            mask2 = mask1.repeat_interleave(2, dim=1).repeat_interleave(2, dim=2)  # [b,1,2h,2w]
            ...
            mask4 = mask1.repeat_interleave(4, dim=1).repeat_interleave(4, dim=2) # [b,1,4h,4w]
        ...
        mask_x1 = torch.stack(mask_x1, dim=0)
        mask_x2 = torch.stack(mask_x2, dim=0)
        mask_x4 = torch.stack(mask_x4, dim=0)
        ...
        mask_scale ={}
        mask_scale['x1'] = mask_x1
        mask_scale['x2'] = mask_x2
        mask_scale['x4'] = mask_x4


        img_ref_feat = self.vgg(img_ref_hr)
        return pre_offset, img_ref_feat, mask_scale
