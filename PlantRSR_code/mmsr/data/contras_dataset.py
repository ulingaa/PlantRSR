...
def image_pair_generation(img,
    ...
    #np.random.randint(10,max(w,w-160-10)) 在(10,w)中随机选择一个值
    x_topleft = np.random.randint(random_perturb_range[1], max(w, w - cropS - random_perturb_range[1]))
    # #np.random.randint(10,max(h,h-160-10)) 在(10,h)中随机选择一个值
    y_topleft = np.random.randint(random_perturb_range[1], max(h, h - cropS - random_perturb_range[1]))
    ...
    # ===== in image-2
    x2_topleft = x_topleft + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])

    y2_topleft = y_topleft + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])

    x2_topright = x_topright + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])
    
    y2_topright = y_topright + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])

    x2_bottomleft = x_bottomleft + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])

    y2_bottomleft = y_bottomleft + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])

    x2_bottomright = x_bottomright + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])
    
    y2_bottomright = y_bottomright + np.random.randint(random_perturb_range[0], random_perturb_range[1]) * np.random.choice([-1.0, 1.0])
...
class ContrasDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img_in_transformed =(img_in,H_inverse)
        img_in_transformed, H, H_inverse = image_pair_generation(
            img_in, (0, 10), 160)

        #transformed_coordinate表示原始图像img_in中的像素坐标经过逆变换后在目标图像中的位置。
        ## grid_x, grid_y = np.meshgrid(np.arange(160), np.arange(160))
        #np.arange(160)=[0,1,2,3,4,...,159]
        #grid_x, grid_y都是[160,160] x是所有列相同，行[0,1,2,3,4,...,159] y是列[0,1,2,3,4,...,159]，所有行相同
        grid_x, grid_y = np.meshgrid(np.arange(gt_w), np.arange(gt_h))
        #grid_z[160,160] 都是1
        grid_z = np.ones(grid_x.shape)
        #[3,160,160]-->[3,25600]坐标值排列成一个序列 
...
class ContrasValDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_in_transformed_pil = img_in_transformed * 255
        img_in_transformed_pil = Image.fromarray(cv2.cvtColor(img_in_transformed_pil.astype(np.uint8), cv2.COLOR_BGR2RGB))

        img_in_lq = img_in_pil.resize((lq_w, lq_h), Image.BICUBIC)
        img_in_transformed_lq = img_in_transformed_pil.resize((lq_w, lq_h),Image.BICUBIC)

        # bicubic upsample lr
        img_in_up = img_in_lq.resize((gt_w, gt_h), Image.BICUBIC)
        img_in_transformed_up = img_in_transformed_lq.resize((gt_w, gt_h),Image.BICUBIC)

        img_in_lq = cv2.cvtColor(np.array(img_in_lq), cv2.COLOR_RGB2BGR)
        img_in_lq = img_in_lq.astype(np.float32) / 255.
        img_in_up = cv2.cvtColor(np.array(img_in_up), cv2.COLOR_RGB2BGR)
        img_in_up = img_in_up.astype(np.float32) / 255.
        img_in_transformed_lq = cv2.cvtColor(np.array(img_in_transformed_lq), cv2.COLOR_RGB2BGR)
        img_in_transformed_lq = img_in_transformed_lq.astype(np.float32) / 255.
        img_in_transformed_up = cv2.cvtColor(np.array(img_in_transformed_up), cv2.COLOR_RGB2BGR)
        img_in_transformed_up = img_in_transformed_up.astype(np.float32) / 255.
